...
ソースの冒頭にいくつか設定があります必要に応じて変更してください。
...
おそらく日本語設定以外の場合はStill(?)等に書き換える必要があると思います。

メディアプール内のタイプが画像のクリップだけを抽出するために指定している名前です。

### ROTATION_ANGLE

縦写真のDNGを回転する角度を指定しています。合わない場合は変更してください。

### ZOOM

縦写真のDNGの拡大率。うまく行かない場合は変更してください。
